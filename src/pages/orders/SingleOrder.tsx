...
const SingleOrder = () => {
	...
	const { data: order } = useQuery<Order>({
		...
		queryFn: async () => {
			...
			return await getSingle(orderId as string, queryString).then((res) => res.data);
		},
	});
	...
	const { mutate } = useMutation({
		...
		mutationFn: async (status: OrderStatus) => {
			return await changeStatus(orderId as string, { status });
		},
		onSuccess: () => {
					...
					<Card title="Customer Details">
						<Space direction="vertical">
							...
							<Flex style={{ flexDirection: "column" }}>
								...
								<Typography.Text>{capitalizeFirst(order.paymentStatus)}</Typography.Text>
							</Flex>
							...
							<Flex style={{ flexDirection: "column" }}>
								...
								<Typography.Text>{format(new Date(order.createdAt), "dd/MM/yyy HH:mm")}</Typography.Text>
							</Flex>
